...
class GFSService:
    ...
    def _score_bait_theory(self, lat: float, lon: float, ts_ms: int, env: Dict[str, Any], history: Dict[str, Any], env_meta: Dict[str, Any]) -> Dict[str, Any]:
        ...
        bait_score = round(presence * 100.0, 1)
        return {
            "intensity": intensity,
            ...
            "bait_percentage": f"{int(bait_score)}%",
    ...
    def derive_balloon_vectors(self, datasets: dict[str, Any], target_ft: int = 10000) -> list[dict[str, Any]]:
        ...
        preferred = [700.0, 750.0, 650.0, 250.0, 300.0]
        level = None
        ...
        if not math.isfinite(level) or level < 10.0:
            level = 300.0
    ...
    def _derive_real_cloud_tiles(self, fields: dict[str, Any], cycle: str, forecast_hour: int) -> list[dict[str, Any]]:
        ...
        y_step = max(1, precip.shape[0] // 120)
        x_step = max(1, precip.shape[1] // 240)
    ...
    def build_scene_payload(self, weather: dict[str, Any], bbox: dict[str, float]) -> dict[str, Any]:
        ...
        try:
            # Boat placeholder for visualization
            boats.append({
                "id": "boat-1",
                "lat": float(bbox.get("south", 0) + bbox.get("north", 0)) / 2.0,
                "lon": float(bbox.get("west", 0) + bbox.get("east", 0)) / 2.0,
                "heading": 45.0
            })
        except Exception:
            pass
    ...
    def _degraded_scene_payload(self, bbox: dict[str, float], reason: str) -> dict[str, Any]:
        ...
        return {
            ...
            "scene": {"clouds": [], "precip": [], "lightning": [], "wind": [], "swell": [], "boats": [], "fish": []},
    ...
    def config(self) -> Dict[str, Any]:
        google_maps_api_key = (os.getenv("GOOGLE_MAPS_API_KEY", "") or os.getenv("MAPS_API_KEY", "")).strip()
        return {
            "enabled": self.state.enabled,
            "api_base": "/gfs/api",
            "ws_base": "/gfs/ws",
            ...
            "runtime": {
                "key_present": bool(google_maps_api_key),
                "env_source": "GOOGLE_MAPS_API_KEY" if os.getenv("GOOGLE_MAPS_API_KEY", "").strip() else ("MAPS_API_KEY" if os.getenv("MAPS_API_KEY", "").strip() else ""),
            },
            "ingest": {
