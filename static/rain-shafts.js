(function () {
  ...
  function columnFootprint(ctx, lat, lon, spread, key) {
    const pts = [];
    const n = 8;
    const lonScale = 1 / Math.max(0.1, Math.cos((lat * Math.PI) / 180));
    for (let i = 0; i < n; i += 1) {
      const a = (Math.PI * 2 * i) / n;
      const r = seeded(ctx, `${key}:r:${i}`, 0.74, 1.28);
      pts.push({ lat: lat + Math.sin(a) * spread * r, lng: lon + Math.cos(a) * spread * lonScale * r });
    }
    return pts;
  }
  ...
  async function renderRainShafts(ctx) {
    ...
    const columns = sourceColumns.length
      ...
      : fallbackTiles
          ...
          .map((t) => ({
            tile_id: t.tile_id,
            lat: Number(t.lat ?? t.bounds?.lat_center ?? 0),
            lon: Number(t.lon ?? t.bounds?.lon_center ?? 0),
