...
function speciesPoolForClass(markerClass = 'coastal') {
  if (markerClass === 'inland' || markerClass === 'inland_freshwater') {
    return [
...
function deriveIntel({ loc, wx, bait, clouds, boats, ocean, localOverlay, reports, videos, profile }) {
  ...
  const baitApplicable = Boolean(loc?.bait_applicable ?? !['inland', 'inland_freshwater'].includes(markerClass));
  ...
  const baitStatus = !baitApplicable ? 'not_applicable' : (Number.isFinite(baitProb) ? 'available' : 'unavailable');
...
export function createHud({ root, onStartLive, onStopLive, onSelectLocation, getOverlaySummary }) {
  ...
  function renderImmediateFromMarker(marker) {
    ...
    const baitApplicable = Boolean(marker?.bait_applicable ?? quick?.bait_applicable ?? !['inland', 'inland_freshwater'].includes(markerClass));
    el.waterbody.textContent = `Curated CSV location marker • ${markerClass}`;
    el.statusLine.textContent = 'Loading detailed intelligence…';
    const quickBaitScore = Number(quick?.bait_score);
    const quickBaitStatus = !baitApplicable ? 'not_applicable' : (Number.isFinite(quickBaitScore) ? 'available' : 'unavailable');
    el.baitSummary.textContent = quickBaitStatus === 'available'
      ? `${quickBaitScore.toFixed(0)}% • ${quick?.bait_intensity || 'warming'}`
      : quickBaitStatus === 'not_applicable' ? 'Not applicable' : 'Unavailable';
    el.envNow.textContent = (markerClass === 'inland' || markerClass === 'inland_freshwater')
  ...
  async function refresh() {
    ...
    el.baitSummary.textContent = intel.baitStatus === 'available'
      ? `${Math.round(Number(intel.baitScore) || 0)}% • ${intel.baitState}`
      : intel.baitStatus === 'not_applicable' ? 'Not applicable' : 'Unavailable';
    updateMeter(el.baitFill, intel.baitStatus === 'available' ? (Number(intel.baitScore) || 0) : 0);
    ...
    const isInland = intel.markerClass === 'inland' || intel.markerClass === 'inland_freshwater';
    if (isInland) {
